...
We therefore extract all available tracts through the \texttt{Butler} using \texttt{lsst-scipipe-9.0.0} and the \texttt{intermittentcumulativeDRP/\allowbreak 20240402\_03\_04/\allowbreak d\_2024\_03\_29/DM-43865} collection.
...
Thus, for these self-similar objects -- same magnitude and hence SNR, same apparent astrometric precision -- we calculate the histogram of cross-match separations to our truth table.
...
\begin{figure}
  ...
  \caption{Comparison between astrometric precision model and distribution of measured-truth position separations.
           ...
           Black lines show expected distribution of separations based on quoted astrometric precisions, while red and cyan lines show different best-fitting precisions, and green and magenta lines show contributions from false-match model.
...
We can see in this case that the pipeline-derived precisions are too low (0.0014 arcsec, 1.4 mas) than the best-fit values (4.4 mas).

At lower brightnesses, we see much better agreement.
Figure \ref{fig:sv_250_2_ap03_22.75} shows the case for $i=22.75$, where we see excellent agreement between our quoted precision and the best-fit parameterisation of the position-position residual distribution.
...
\begin{figure*}
  ...
  \caption{Comparison between best-fit individual and pipeline-derived precisions.
           All magnitude slices for each tract are shown, with decreasing brightness tracking towards the top-right of the figures, at lower SNR.
...
Figure \ref{fig:ind_sig-sig_source} summarises these results, plotting the individually derived astrometric precision that best fits the cross-match residuals against the quoted astrometric precision for the detections.
...
As discussed previously, there are two different versions of each catalogue that the LSST pipeline will produce: the per-visit, time-series-enabling \texttt{Source} table, and the deep-stack, coadd \texttt{Object} table.
...
Both our best-fit individual astrometric precision (cyan line) and quadratic-fit hyper-parameterisation (red line) are in good agreement with the scatter of cross-match separations.
...
However, as shown in Figure \ref{fig:3384_ap03_25.75}, this agreement breaks down at faint magnitudes.
Here we see a reversal of what we saw previously, in the individual-slice best-fit precision being smaller than the quoted value, and the quadratic scaling law no longer working.
This is fainter than the single-visit completeness limit, so beyond the scope of comparison with the \texttt{Source} table results, and hence requires further investigation.

The quadratic-fit scaling relation breakdown has a relatively simple explanation.
...
This both confirms, in a small way, the photometric portion of the LSST pipeline, but also validates our visit cuts made previously.
...
\begin{figure}
  ...
  \caption{Astrometric precision-magnitude relationship, for \texttt{Source} table objects in the ``SV 225 -40'' pointing.
         ...
         Arrows show the change in astrometric precision required to explain cross-match separation scatter for each magnitude slice bin, with arrows going from ``quoted'' precision to ``best-fit'' precision.}
...
The theoretical relationship between the SNR of a source and the precision with which you can determine its centroid on a detector is derived by \citet{King:1983aa}, and scales as $\sigma \propto a / \mathrm{SNR}$, with $a$ the ``sigma'' of the Gaussian approximation to the Airy disk seeing profile, related to its full-width at half-maximum (FWHM), the so-called ``seeing'' quality.
...
A few representative seeings are overlaid on Figure \ref{fig:225_-40_snr_sig_scaling}, 0.8 to 1.2 arcsec -- roughly ``good'' to ``poor'' seeing for the Rubin Observatory site -- and we can a suggestion that these particular visits were simulated with seeing on the better end of the range.
...
The analysis performed on the merged-tract data suggests that the relationship between quoted and ``scatter-based'' uncertainties holds across the whole sky, if one takes SNR to be the ``true'' parameter of pipeline performance.
...
So while we caution against blind faith in the conclusions drawn, and suggest independent verification of the two potential causes of \texttt{Object} table astrometry tension, we tentatively suggest that SNR may be the ``tiebreaker'' which confirms the covariances as being incorrectly determined.
